...
class SaveBoxWidget(QWidget, Ui_SaveBoxWidget):
    ...
    def select_data_root(self):
        ...
        data_root_string = get_abbreviated_path_string(self.data_root_path, max_len=35)
    ...
    def select_data_path(self):
        ...
        data_path_string = get_abbreviated_path_string(self.data_path)
        self.data_dir_label.setText(data_path_string)

    def build_file_name(self):
        self.file_prefix = self.file_prefix_lineEdit.text()
        self.file_number = self.file_number_spinBox.value()
        self.file_suffix = '.h5'
        self.file_path = Path(f'{self.file_prefix}_{self.file_number:05d}{self.file_suffix}')
        self.file_path_label.setText(f'Next File Name: {str(self.file_path)}')

    def get_full_file_path(self):
        self.full_file_path = Path(self.data_path, self.file_name)
        return self.full_file_path

    def get_savepath(self):
        self.data_root = Path(self.data_root_lineEdit.text())
        self.date_path = self.get_date_path()
        self.run_path = self.get_run_path(self.run_name_lineEdit.text())
    ...
    def get_date_path():
        """
        Return folder path in the format 'YYYY\MM\DD'
        """
        today = datetime.datetime.today()
        year = f'{today.year:4d}'
        month = f'{today.month:2d}'
        day = f'{today.day:2d}'
        date_path = Path(year, month, day)
        return date_path

    @staticmethod
    def get_run_path(runname):
        """
        Data directory structure includes a 'data' folder in every daily folder
        """
        run_path = Path('data', runname)
        return run_path
