...
class AndorCamera(object):
	...
	def get_software_version(self):
		...
		dllRev = c.c_int()
		dllVer = c.c_int()
		error = dll.GetSoftwareVersion(c.byref(eprom), c.byref(cofFile), c.byref(vxdRev), c.byref(vxdVer),  c.byref(dllRev), c.byref(dllVer))
		if ERROR_CODE[error] != 'DRV_SUCCESS':
			raise Exception(ERROR_CODE[error])
	...
	def set_image(self, hbin, vbin, hstart, hend, vstart, vend):
		...
		error = dll.SetImage(c.c_int(hbin), c.c_int(vbin), c.c_int(hstart), c.c_int(hend), c.c_int(vstart), c.c_int(vend))
		if (ERROR_CODE[error] == 'DRV_SUCCESS'):
